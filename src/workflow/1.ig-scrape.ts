...
export async function scrape() {
  const usernames = ['infolomba.indonesia.id', 'csrelatedcompetitions']
  ...
  for (const username of usernames) {
    try {
      ...
      const results = await scraper.getPosts(username, 6);
      ...
      if (error instanceof ScrapeError) {
        console.error(`Scraping error: ${(error as ScrapeError).message} (${(error as ScrapeError).code})`);
